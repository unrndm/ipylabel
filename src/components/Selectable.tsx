...
const Selectable = ({
  ...
  const handleSelection = (event: any) => {
    ...
    const first_end = parseInt((selection.getRangeAt(0).startContainer.parentNode! as HTMLElement).dataset.position!)
    const second_end = parseInt((selection.getRangeAt(0).endContainer.parentNode! as HTMLElement).dataset.position!)
    ...
    <div onMouseUp={handleSelection} onDoubleClick={handleSelection}>
      {text.split("").map((letter, idx) => {
        ...
        if (typeof label_as === "string") {
          ...
          return <span data-position={idx}>{letter}</span>;
        }
      })}
    </div>
  );
